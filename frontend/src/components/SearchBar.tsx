...
const logoMap: Record<string, string> = {
  ...
  instagram: "/Logos/instagram.png", // REMPLACÉ
  x: "/Logos/X-Logo.png"
};
...
export default function SearchBar({ onSearch, isLoading = false }: SearchBarProps) {
  ...
  const detectPlatform = (link: string) => {
    ...
    if (lower.includes("instagram.com")) return setPlatform("instagram"); // REMPLACÉ
  ...
  return (
    <div className="w-full max-w-2xl relative z-10">
      ...
      <div className="relative group">
        <div className={`absolute -inset-1 rounded-xl opacity-20 group-hover:opacity-40 transition duration-500 blur-lg ${platform === 'youtube' ? 'bg-red-600' :
            platform === 'tiktok' ? 'bg-cyan-500' :
              platform === 'instagram' ? 'bg-pink-600' : // MODIFIÉ
                'bg-blue-600'
          }`}></div>
